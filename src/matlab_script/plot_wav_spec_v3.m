...
% % 
   method = 'D:\��\Heartlung\data\heart_lung_sam2\mix';
...
a="D:\Git\qpzm_mfa_dae\src\senpai_data\heart_lung_sam2\mix\training_clean_心跳\0dB\4_1.wav"
...
shrink=0;
shift=0;
[CleanSpec,~,yphase] = Spectrum(audioData*500,FrameLength,FrameRate,FFT_SIZE, flag);
...
set(gca, 'YTick',[0,125,4000]);
set(gca,'YTickLabel',{'0','500','1050'})
imresize(h_pc, [20 40]);  
colormap Jet;axis xy;box off;
set(gca, 'CLim', [1 10]);%frequency band
% fig=imagesc(spectrogram(1:70,:)); axis xy; colormap('jet');%xlabel('Frame index');
...
% audioData = audioread(a);
% 
% [CleanSpec,~,yphase] = Spectrum(a*500,FrameLength,FrameRate,FFT_SIZE, flag);
% log10powerspectrum  =log10(CleanSpec);fs=sr;
% sig=PowerSpectrum2Wave(log10powerspectrum,yphase);
% 
% 
